...
if [ ! -z "$dir" ]; then
	...
	nint=$(grep -E 'bond0|bond1|eno3' $(ls ${dir}/log_sar* | awk 'NR==1{print}')| awk '{print NF}' | grep 9 | wc -l)
	# echo "interface: $nint"
	# nn=($(grep -n 'IFACE' log_sar07 | awk -F: -e 'NR==1{printf "%s ",$1}' -e 'NR==2{print $1}'))
	# nn=$(grep -n 'IFACE' log_sar07 | awk -F: '{print $1}')

	# n=$(grep -n -m1 'bond1' log_sar07 | awk -F: '{print $1}')
